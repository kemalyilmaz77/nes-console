...
class JoystickView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    double actualSize = size ?? _math.min(MediaQuery.of(context).size.width,
      ...
      child: StatefulBuilder(
        builder: (context, setState) {
          ...
          return GestureDetector(
            onPanStart: (details) {
              callbackTimestamp = _processGesture(actualSize, actualSize / 2,
                  details.localPosition, callbackTimestamp!);
            ...
            child: (opacity != null)
                ? Opacity(opacity: opacity??1, child: joystick)
                : joystick,
          );
        },
      ),
  ...
  DateTime _processGesture(double size, double ignoreSize, Offset offset,
    ...
    DateTime callbackTimestamp0 = callbackTimestamp;
    if (onDirectionChanged != null &&
        _canCallOnDirectionChanged(callbackTimestamp)) {
      callbackTimestamp0 = DateTime.now();
      onDirectionChanged?.call(degrees, normalizedDistance);
    }

    return callbackTimestamp0;
  }
